...
// ─────────────────────────────────────────────────────────────────────────────
// HTML — ditulis sebagai string biasa (bukan template literal) untuk
// menghindari konflik backtick / escape di dalam Workers
// ─────────────────────────────────────────────────────────────────────────────
function getHTML() {
  return [
    '<!DOCTYPE html>',
    ...
    '.sbar{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;padding:12px 24px;background:#0a0a12;border-bottom:1px solid #1e1e2e}',
    ...
    '.pcard{background:#12121a;border:1px solid #1e1e2e;border-radius:13px;margin-bottom:12px;overflow:hidden}',
    ...
    '.bdg{font-size:.55rem;padding:2px 6px;border-radius:4px;font-weight:700}',
    '.epc{font-size:.6rem;color:#64748b;font-family:monospace}',
    '.bpath{font-size:.58rem;color:#374151;font-family:monospace}',
    '.chev{font-size:.65rem;color:#475569;transition:transform .25s}',
    '.chev.open{transform:rotate(180deg)}',
    '.epgrid{display:none;border-top:1px solid #1e1e2e;padding:14px;gap:10px;grid-template-columns:repeat(auto-fill,minmax(340px,1fr))}',
    ...
    '.eph{display:flex;align-items:center;gap:8px;padding:9px 13px;border-bottom:1px solid #1a1a28}',
    '.mth{font-size:.52rem;font-weight:700;padding:2px 6px;border-radius:3px;background:#14532d;color:#4ade80;font-family:monospace}',
    '.epath{font-family:monospace;font-size:.68rem;color:#94a3b8}',
    '.ename{margin-left:auto;font-size:.58rem;color:#475569}',
    ...
    '.pn{font-family:monospace;font-size:.62rem;color:#c084fc;min-width:80px}',
    ...
    '.epurl{padding:7px 13px;border-bottom:1px solid #1a1a28}',
    '.ulbl{font-size:.5rem;color:#374151;text-transform:uppercase;letter-spacing:1px;margin-bottom:3px}',
    '.uprev{background:#060609;border:1px solid #1a1a28;border-radius:4px;padding:5px 8px;font-family:monospace;font-size:.56rem;word-break:break-all;display:flex;justify-content:space-between;align-items:flex-start;gap:6px}',
    '.utext{flex:1;line-height:1.5}',
    ...
    '.rtime{font-size:.55rem;color:#374151;font-family:monospace;margin-left:auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px}',
    ...
    '  <div class="stat"><div class="sn" id="s-hits">--</div><div class="sl">Total Hits</div></div>',
    '  <div class="stat"><div class="sn" id="s-vis">--</div><div class="sl">Visitors</div></div>',
    '</div>',
    '<div class="toolbar">',
    ...
    '  if(btn){btn.textContent="Copied!";setTimeout(function(){btn.textContent="Copy URL";},1200);}',
    '}',
    ...
    'function xpandAll(){',
    '  xAll=!xAll;',
    '  document.querySelectorAll(".epgrid").forEach(function(g){g.classList.toggle("open",xAll);});',
    '  document.querySelectorAll(".chev").forEach(function(c){c.classList.toggle("open",xAll);});',
    ...
    '    if(s.total_hits!=null)document.getElementById("s-hits").textContent=s.total_hits.toLocaleString();',
    '    if(s.unique_visitors!=null)document.getElementById("s-vis").textContent=s.unique_visitors.toLocaleString();',
    '  }).catch(function(){});',
    '}',
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Health check
    if (path === "/health") {
      return jsonRes({ status: "ok", worker: "Sonzai X API Proxy", target: TARGET_BASE, ts: new Date().toISOString() });
    }
